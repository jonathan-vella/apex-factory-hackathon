...
3. `app/README.md` describes the legacy starting state:
   ...
   - A package section: the workflow, the `legacy-v1` release, that B04 deploys it to `vm-app01`, and that the workflow adds the missing Bootstrap CSS and the five unreferenced runtime DLLs (requirement 5) to the package only.
...
5. Package: stage the web app without source (no `obj`, `packages`, `Properties`, `.vs`, `*.cs`, `*.csproj`, `*.user`, `*.sln`, `packages.config`, `*.md`, `.gitignore`), copy `bootstrap.css` and `bootstrap.min.css` from the restored `bootstrap` 5.3.3 NuGet package (`packages/bootstrap.5.3.3/content/Content`) into the package's `Content` folder, copy the DLLs of the five packages that `packages.config` lists but the project file doesn't reference (`Microsoft.Bcl.HashCode`, `System.Buffers`, `System.Memory`, `System.Numerics.Vectors` and `System.Runtime.CompilerServices.Unsafe`, at their `packages.config` versions, except `System.Runtime.CompilerServices.Unsafe`, which is 4.7.1 (restored in CI with `nuget install`), from `lib/net461`, `lib/net46` or `lib/netstandard2.0`, in that order) into the package's `bin` folder, and zip it as `ContosoUniversity-legacy.zip`.
6. Check the package before publishing it. It must contain `Web.config`, `Global.asax`, `Views/Web.config`, `Views/Home/Index.cshtml`, `bin/ContosoUniversity.dll`, `Content/bootstrap.css`, `Content/Site.css` and the five DLLs from requirement 5 in `bin`, and no `.cs`, `.csproj` or `.sln` file. On success, print `Package OK: <n> entries.`; otherwise fail and list what's missing or not allowed.
...
- **Unreferenced runtime DLLs:** upstream's `packages.config` lists five packages that the project file doesn't reference, so msbuild never copies them to `bin`, and the site returns HTTP 500 (`Could not load Microsoft.Bcl.HashCode`). Only the package gets them (requirement 5). `Web.config` redirects `System.Runtime.CompilerServices.Unsafe` to assembly 4.0.6.0, which `Microsoft.Extensions.Primitives` 3.1.32 is also built against, but `packages.config` pins package 4.5.3 (assembly 4.0.4.1). So the package uses Unsafe 4.7.1 (assembly 4.0.6.0), as the owner approved. `Microsoft.Data.SqlClient` also references `Microsoft.IdentityModel.*` and `System.IdentityModel.Tokens.Jwt`, which upstream doesn't list at all. They're only loaded for Always Encrypted enclave attestation, which the app doesn't use, so the package leaves them out.
