...
- **Plan and execute prompts as agent skills (owner-approved, 2026-09-25).** `.github/skills/modernize-plan/SKILL.md` and `.github/skills/modernize-execute/SKILL.md` (`disable-model-invocation: true`, run by slash command only) hold the plan and execute prompts, and replace **Create Plan** and the correction reply. They first shipped as VS Code prompt files that pinned the agent and model; those were replaced by skills because the Copilot harness, which the assessment dashboard switches to, doesn't load prompt files. Skills can't pin an agent or model ([Use Agent Skills in VS Code](https://code.visualstudio.com/docs/agent-customization/agent-skills)), so the protocol names them and the owner picks them: `modernize`, GPT-6 Sol at Medium to plan, GPT-6 Luna at maximum to execute. This changes [B06](../../backlog/B06-spike-ghcp.md) requirement 7 and its Deliverables.
