...
14. **The prompt-file pins may not apply with Run Prompt.** The chat footer in the owner's screenshot showed agent `Agent` and model `GPT-5.6 Sol`, while the file pins `agent: modernize` and `model: ["GPT-6 Sol (copilot)"]`. Pending the owner's confirmation of what the footer showed; run 2 checks it in step 3.1. If the pins don't apply, B10's skills can't rely on them either.
