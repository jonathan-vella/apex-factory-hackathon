...
> [!WARNING]
> **BEFORE you start execution:** in the Chat panel, check that the model picker shows **GPT-6 Luna** with reasoning **maximum**, and that the agent is **`modernize`**. Execution uses whatever is selected.
>
> **How to check:** the agent and model pickers sit at the bottom of the chat input box in the Chat panel.

4. ⚠️ **BEFORE you start:** in the Chat panel, check the model picker shows **GPT-6 Luna** with reasoning **maximum**, and the agent is **`modernize`**. (The pickers are at the bottom of the chat input box.) **(v2)** If this VS Code window saw any abandoned plan or execution attempt, start execution in a **new chat** (and reload the window if it still fails): send `Execute the modernization plan in .github/modernize/<plan-folder>`. Otherwise start execution of the checked plan from the dashboard or the plan's chat. Note the exact button or prompt you used in the commit body. If the `execution-coordinator` returns **BLOCKED** because it "saw an in-progress execution-phase tracker", nothing was changed: start again in a new chat. It runs the plan task by task; reply `continue` when it asks. Keep the changes when they build. Do the step 3 check below after the upgrade task, and the step 4 checks after each of the other tasks, committing after each task with the step names below and exporting the chat before each commit.
