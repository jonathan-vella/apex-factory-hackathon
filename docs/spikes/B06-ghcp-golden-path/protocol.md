...
- **(v2) Run the app after every task, before you commit.** `dotnet run` and open a page. The agents' validations build but never start the app: in run 1, the OpenTelemetry task passed its validation and then crashed at startup.
- **(v2) Keep the agent in the app.** End every prompt you type with `Scope: app/ContosoUniversity only; never edit files outside it.` Before each commit, check `git status --short` for changes outside `app/` and `.github/modernize/`, and revert them (`git restore <path>`). In run 1, the OpenTelemetry task's completeness check scanned the whole repo and edited the kit's `docs/prd.md`.
...
4. ⚠️ **BEFORE you start:** in the Chat panel, check the model picker shows **GPT-6 Luna** with reasoning **maximum**, and the agent is **`modernize`**. (The pickers are at the bottom of the chat input box.) **(v2) Plan and execute in separate chats, one task per chat.** Execution never runs in the chat that planned it: nested coordinators in one chat use up the sub-agent depth limit. For **each task** in `tasks.json`, in order:
   ...
   5. **(v2)** Run **Developer: Reload Window** from the Command Palette, then start the next task in another new chat. **Expect the first attempt of a task to stop on a tracker: that isn't your error.** In run 1, tasks 003 and 005 both stopped on the first attempt, even in a fresh chat after a reload, because the agent saw an in-progress handoff or tracking record, sometimes one it had just created itself. When it offers **Retry task 00N**, pick **Retry**; if it doesn't offer it, reply `Mark the stale tracking item complete and run task 00N.` The retry runs the task. **(v2) If the retry says "no modernization scenario is active"** (the scenario context can be lost across window reloads), don't let it initialize a new scenario: that starts a new scenario instead of resuming the plan. Instead, send a **direct prompt**: a new chat with **`modernize`** and **GPT-6 Luna** at **maximum** (if `modernize` still blocks, the **default Agent** is the fallback). Send `Implement task 00N of the modernization plan directly on spike/b06-runN. Work directly: don't delegate, don't initialize a scenario and don't re-plan. Scope: app/ContosoUniversity only; never edit files outside it. The task's requirements from .github/modernize/<plan-folder>/.metadata/tasks.json:` and paste the task's `description` and `requirements`. Then commit and push as in step 3.4.3. Use the same fallback if `modernize` says the task "is already being handled by another agent": in run 1 that came with no agent ID and no changes.
...
Set the values, then create or edit a student and check both directions. **(v2)** The **Notifications** page is only an information page: notifications show up as toasts in the top-right corner of every page, polled every 5 seconds. To check the receive path directly, open `/Notifications/GetNotifications` in the browser: it must return `"success":true`. `{"success":false,"message":"Error retrieving notifications"}` means the receive path is broken, even when sending works (run 1: `ReceiveMessagesAsync` was called with `TimeSpan.Zero` as the maximum wait time, which must be positive). Check the queue from the CLI too: `az servicebus queue show -g rg-spike-b06 --namespace-name sbns-uni-<suffix>-b06 -n notifications --query countDetails.activeMessageCount`.
...
3. **`Trace` to OpenTelemetry:** **(v2)** use the predefined task **Tasks** > **.NET** > **Logging or Observability Tasks** > **Migrate Logging or Observability to OpenTelemetry on Azure**, with `modernize` and GPT-6 Luna at maximum. In run 1 it covered `System.Diagnostics.Trace`: it replaced the `Trace` and `Debug` calls with `ILogger` and added Azure Monitor OpenTelemetry. It also edited `docs/prd.md`, outside the app: revert that before you commit (see **Keep the agent in the app**). After it, check that the app starts without `APPLICATIONINSIGHTS_CONNECTION_STRING`: in run 1 it crashed at startup ("A connection string was not found"), because the task registers Azure Monitor unconditionally, and the task's own validation passed without starting the app. If it crashes, send: `Register Azure Monitor only when APPLICATIONINSIGHTS_CONNECTION_STRING or ApplicationInsights:ConnectionString is set; otherwise use the console exporter. Scope: app/ContosoUniversity only; never edit files outside it.` If the predefined task isn't available, use:
...
2. Send the assessment and planning prompt. It uses the same seven kit rules as run 1's planning prompt (step 3.2):
...
3. Build, then do the same check as run 1 for that stage: step 3.5 (pages against `10.10.n.4`) after the upgrade, then the step 4.1–4.4 checks (Students against `10.10.n.4`, a Blob upload, Service Bus send **and** receive with `/Notifications/GetNotifications`, and Key Vault).
...
| Comparison | **Owner-approved:** after run 1, a full end-to-end run with GitHub Copilot upgrade (`ms-dotnettools.upgrade-agent`, Upgrade agent; GPT-6 Sol Medium to assess and plan, GPT-6 Luna maximum to execute) on `spike/b06-upgrade-compare` from `da4e5f6`, with GitHub Copilot modernization disabled by default, compared stage by stage in `compare-upgrade.md` | The `modernize` agent hands version upgrades to GitHub Copilot upgrade, which isn't installed; run 1's task 001 was done without it (findings 19 and 30) |
| 3.4.5 | Fallback for any blocked task: the default Agent with GPT-6 Luna maximum, the task's `description` and `requirements` pasted from `tasks.json`, "work directly, don't delegate", then commit and push. Supersedes the previous row's direct `modernize` prompt | A direct `Implement task 005 from tasks.json` prompt to `modernize` was blocked a third way: "task 005 is already being handled by another agent", with no agent ID or result and no changes |
| 3.4.5 | The direct prompt goes to `modernize` first; the default Agent is the fallback. Supersedes the previous row's default-Agent-first order | Task 005 succeeded first time with `modernize` and the direct "work directly, don't delegate" prompt: the "Execute the plan" delegation was what blocked |
| Rules, 3.4.5, 5.3 | End every prompt with `Scope: app/ContosoUniversity only; never edit files outside it.` and revert changes outside `app/` and `.github/modernize/` before each commit. Step 5.3 uses the predefined OpenTelemetry task, which covers `Trace` | Run 1's OpenTelemetry task replaced `Trace` and `Debug` with `ILogger`, but its repo-wide completeness check also edited the kit's `docs/prd.md` |
