...
4. ⚠️ **BEFORE you start:** in the Chat panel, check the model picker shows **GPT-6 Luna** with reasoning **maximum**, and the agent is **`modernize`**. (The pickers are at the bottom of the chat input box.) **(v2) Plan and execute in separate chats, one task per chat.** Execution never runs in the chat that planned it: nested coordinators in one chat use up the sub-agent depth limit. For **each task** in `tasks.json`, in order:
   ...
   5. **(v2)** Run **Developer: Reload Window** from the Command Palette, then start the next task in another new chat. **Expect the first attempt of a task to stop on a tracker: that isn't your error.** In run 1, tasks 003 and 005 both stopped on the first attempt, even in a fresh chat after a reload, because the agent saw an in-progress handoff or tracking record, sometimes one it had just created itself. When it offers **Retry task 00N**, pick **Retry**; if it doesn't offer it, reply `Mark the stale tracking item complete and run task 00N.` The retry runs the task.
