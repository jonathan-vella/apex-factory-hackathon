...
1. **(v2)** In the Chat view, set the **Session Target** (harness) to **Local**, not **Agent Host** (**Copilot**): Agent Host doesn't load prompt files, so the pins don't apply and the chat falls back to the default agent and model. Then set the reasoning effort for the planning model in the picker. Open `.github/prompts/modernize-plan.prompt.md` in the editor and select **▶ Run Prompt** in the editor title bar (or run **Chat: Run Prompt** from the Command Palette and pick `modernize-plan`). Typing `/modernize-plan` in chat also works in the Local harness; in run 1 it failed only because the session was on Agent Host. It runs the `modernize` agent with the planning model and the full plan prompt: .NET 10 first, no Entra ID, Blob, Managed Instance with local SQL auth kept, the order database, Blob, Service Bus, Key Vault, `DefaultAzureCredential` with no keys, App Service for Linux containers, the configuration keys and no provisioning. It plans and waits. **(v2)** Check the agent and model the chat shows (the chat input footer or the response header): they should be `modernize` and the model in the table. In run 1 the footer showed `Agent` and `GPT-5.6 Sol` because the session was on Agent Host. Write what it showed in the commit body; if it doesn't match, check the harness is **Local** first, and note it. If the plan asks which Azure resources to use for integration tests, answer: configuration keys supplied at execution time, no identifiers.
