...
4. ⚠️ **BEFORE you start:** in the Chat panel, check the model picker shows **GPT-6 Luna** with reasoning **maximum**, and the agent is **`modernize`**. (The pickers are at the bottom of the chat input box.) **(v2) Plan and execute in separate chats.** Start execution in a **new chat**, never in the chat that planned it: nested coordinators in one chat use up the sub-agent depth limit. Send `Execute the modernization plan in .github/modernize/<plan-folder>, starting with task 001.` Note the exact prompt in the commit body. If execution is **BLOCKED** (for example "saw an in-progress execution-phase tracker" or "Maximum sub-agent depth of 4 reached"), nothing was changed: abort, reload the window (**Developer: Reload Window**), and start again in a new chat. Never retry in the same chat. It runs the plan task by task; reply `continue` when it asks. Keep the changes when they build. Do the step 3 check below after the upgrade task, and the step 4 checks after each of the other tasks, committing after each task with the step names below and exporting the chat before each commit.
