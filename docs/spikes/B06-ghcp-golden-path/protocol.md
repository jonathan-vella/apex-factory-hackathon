...
Model: the assessment uses the extension's default model. **(v2)** Steps 2 and 3 use the dashboard buttons in the **GitHub Copilot modernization** view, which write the prompts for you: **custom assessment**, then **Create Plan** from the report, then execute the plan. That's the owner's chosen path.
...
> [!WARNING]
> **BEFORE you click Create Plan:** in the Chat panel, check that the model picker shows **GPT-6 Sol** with reasoning **Medium**, and that the agent is **`modernize`**. The button uses whatever is selected.
>
...
> [!WARNING]
> **Always send the kit rules prompt below straight after Create Plan, even if the plan looks right.** **Create Plan** doesn't know the kit's rules: run 1's plan had no .NET 10 upgrade, both storage and both database alternatives, an Entra ID sign-in task, no local SQL auth, no hosting target and no rule against keys. Keep the chat on **`modernize`** with **GPT-6 Sol** at **Medium** when you send it.

4. ⚠️ **Always send this, even if the plan looks right.** Check the Chat panel still shows **`modernize`** and **GPT-6 Sol** at **Medium**, then paste this into the plan's chat and send it:

   ```text
   Before execution, revise plan.md and tasks.json so they follow these rules exactly, whatever the plan says now:
   ...
   Then show a table with one row per rule (1-7) and how the revised plan meets it, and wait for my review.
   ```

5. **(v2) Check the table.** All 7 rows must be met, and `plan.md` and `tasks.json` must match them. If any row isn't met, send the prompt again. Edit the plan files by hand only for a small detail, and note what you changed. Export the chat and commit the plan: `run1: step 3 plan` (run 2: `run2: …`, as everywhere). In the commit body, note the agent, model and reasoning effort the chat showed, and how many times you sent the kit rules prompt.

> [!WARNING]
> **BEFORE you start execution:** in the Chat panel, check that the model picker shows **GPT-6 Luna** with reasoning **maximum**, and that the agent is **`modernize`**. The button uses whatever is selected.
>
> **How to check:** the agent and model pickers sit at the bottom of the chat input box in the Chat panel.

6. ⚠️ **BEFORE you click:** in the Chat panel, check the model picker shows **GPT-6 Luna** with reasoning **maximum**, and the agent is **`modernize`**. The button uses whatever is selected. (The pickers are at the bottom of the chat input box.) Then start execution of the reviewed plan from the dashboard or the plan's chat (note the exact button or reply you used in the commit body). It runs the plan task by task; reply `continue` when it asks. Keep the changes when they build. Do the step 3 check below after the upgrade task, and the step 4 checks after each of the other tasks, committing after each task with the step names below and exporting the chat before each commit.
