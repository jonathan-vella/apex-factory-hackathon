...
Upstream doesn't include the Bootstrap CSS that `BundleConfig.cs` and the project reference. The workflow adds `bootstrap.css` and `bootstrap.min.css` from the restored `bootstrap` 5.3.3 NuGet package to the package only. Upstream's `packages.config` also lists five runtime libraries that its project file never references, so the build doesn't copy them to `bin`: `Microsoft.Bcl.HashCode`, `System.Buffers`, `System.Memory`, `System.Numerics.Vectors` and `System.Runtime.CompilerServices.Unsafe`. Without them, the site returns HTTP 500. The workflow copies their DLLs from the restored packages into the package's `bin` folder. For `System.Runtime.CompilerServices.Unsafe` it uses version 4.7.1 (assembly 4.0.6.0), because the app's `Web.config` redirects to that version and `packages.config` pins an older one. The source here stays unchanged.
